...
#process settings
process_video = False # create a video
process_train = False  # train network
process_load  = False  # load trained network
process_save  = False   # save trained network
...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    output = tf.layers.conv2d_transpose(output, num_classes, 16, 8, 'same',
                                        kernel_initializer=tf.truncated_normal_initializer(stddev=1e-2),
                                        kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))  # scale up by x8 to get original image size
